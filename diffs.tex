...
%DIF ADD paper.tex     Tue Nov 10 16:44:38 2015
...
produce synthetic spectra from }\DIFdelend \DIFaddbegin \DIFadd{comparing ``average'' AGN spectra to 
...
assuming total luminosities as high as 10$^{45}$ ergs s$^{-1}$}\DIFaddend . 
We find that a simple treatment of clumping (`microclumping') 
allows for a more realistic \DIFdelbegin \DIFdel{X-ray luminosity in the model}\DIFdelend \DIFaddbegin \DIFadd{AGN luminosities}\DIFaddend , while maintaining the ionization 
...
equilibrium (non-LTE). The code \DIFaddbegin \DIFadd{can be used to model a variety of
...
\DIFdelbegin \DIFdel{To treat line transfer , }\DIFdelend \DIFaddbegin \DIFadd{Our approach to line transfer is based upon the macro-atom implementation developed by \mbox{%DIFAUXCMD
\cite{lucy2002, lucy2003}
}%DIFAUXCMD
. In our case, for reasons of computational effieciency, }\DIFaddend we adopt the  \DIFdelbegin \DIFdel{same hybrid }\DIFdelend \DIFaddbegin \DIFadd{hybrid macro-atom }\DIFaddend scheme 
described by M15 
\DIFdelbegin \DIFdel{, 
}\DIFdelend %DIF > (an improvement over that used by H14 for our earlier quasar study), 
...
or full `macro-atoms'. 
\DIFdelbegin \DIFdel{The macro-atom implementation 
is described in full by \mbox{%DIFAUXCMD
\cite{lucy2002, lucy2003}
}%DIFAUXCMD
.
}\DIFdelend Our scheme allows one to treat non-LTE line transfer in radiative equilibrium
...
}\DIFdelend \DIFaddbegin \DIFadd{, and because we were especially interested in the contribution to AGN spectra of }\LA\DIFadd{.  In H13, all atoms were treated in a two-level approximation.
}\DIFaddend 
...
{\em microclumping} \citep{hamann1998}\DIFaddbegin \DIFadd{(MORE REFS)}\DIFaddend . The key assumption here is that 
...
variables $r_{min}$, $\theta_{min}$, $f$, $\alpha$ and \DIFdelbegin \DIFdel{$R_V$.
...
\begin{itemize}
\item Does the model \DIFdelbegin \DIFdel{maintain the right ionization state to }\DIFdelend \DIFaddbegin \DIFadd{avoid over-ionization and thus }\DIFaddend produce strong BALs \DIFaddbegin \DIFadd{with $BI > ??$}\DIFaddend ?
\item Does significant line emission emerge at low inclinations\DIFaddbegin \DIFadd{, with $EW\sim50\AA$ in }\civ\DIFaddend ?
...
\end{itemize}
\DIFaddbegin \DIFadd{We arrived at a model
}\DIFaddend In this section, we present one of the most promising models and discuss
...
\DIFaddbegin \DIFadd{The full grid, including output synthetic spectra and plots can be found at
...
\DIFaddFL{$f$ 	}&	 \DIFaddFL{$0.01$ }& \DIFaddFL{$0.1$ }& \multicolumn{2}{|l|}{$1$}  \\
\hline 
\end{tabular}
...
\DIFaddend \begin{figure*} %fullpage
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1.0\textwidth]{figures/link.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.0\textwidth]{figures/link8.png}
\DIFaddendFL \caption
{
\DIFdelbeginFL \DIFdelFL{Physical }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Contour plots showing the logarithm of some important 
physical }\DIFaddendFL properties of the outflow\DIFdelbeginFL \DIFdelFL{, shown by the coloured contours}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{. Symbols are defined in the text}\DIFaddendFL .
...
\DIFaddbeginFL \DIFaddFL{The line luminosities represent the luminosity of photons
...
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.5\textwidth]{figures/agnspec.png}
%DIFDELCMD < \caption
%DIFDELCMD < {
%DIFDELCMD < %%%
...
significant fine-tuning when considering a }\DIFdelend \DIFaddbegin \DIFadd{One obvious potential solution to this problem is to hypothesize a more isotropic distribution 
...
As a result, 
}\DIFdelend \DIFaddbegin \DIFadd{However, we have verified using AGNSPEC (REFs to Hubeny)
...
\begin{figure*} %fullpage
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1.0\textwidth]{figures/lx.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.0\textwidth]{figures/lx_a05_pre.png}
\DIFaddendFL \caption
{
...
{\bf \DIFadd{Note: One of the things I definitely wondered about is whether our "model" would produce the fraction of BALs seen in the sky}}
