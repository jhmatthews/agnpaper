...
Various unification schemes have been proposed to interpret the complex phenomenology of quasars and luminous active galactic nuclei (AGN) in terms of  a simple axisymmetric picture involving a central black hole, an accretion disc and an associated outflow.
Here, we continue our tests of  this paradigm by comparing ``average'' AGN spectra to synthetic spectra of simple biconical disc wind models, using  with our Monte Carlo radiative transfer and photoionization code {\sc Python}.
In particular, we investigate whether clumping of the outflow allows us to produce synthetic spectra in the rest-frame UV that have the characteristics of AGN, assuming total luminosities as high as 10$^{45}$ ergs s$^{-1}$. 
We find that a simple treatment of clumping (`microclumping') 
allows for a more realistic AGN luminosities,
while maintaining the ionization state necessary for strong BAL features in the rest-frame UV. 
...
At the highest inclinations, the synthetic spectra possess prominent \mg\ and \al\ BALs, the absorption 
...
% remarkably similar of line-to-continuum ratios apparent 
seen in BAL and non-BAL quasar composites. {\bf This sentence needs to be more precise, especially in light ot the statement earlier in the abstract about ``good agreement''}
This is due to a fundamental constraint arising
from the anisotropy of emission from a classical thin disc. 
We briefly explore the impact of general relativistic effects
...
% relativistic beaming, gravitational redshift and 
% light bending on the angular distribution of disc continuum emission. 
% We find that these general relativistic effects are not yet sufficient to produce a
...
to that described by \cite{shakurasunyaev1973}.  The lines are usually attributed to scattering and absorption of material above the disk.  {\bf Note: the first half of this paragraph and the second do not follow from one another.  The first half is about spectra the second about outflows and the connection to spectra is not made until the next paragrph.  This needs fixing.}
...
{\bf This needs an introductory paragraph.  Should state our earlier modeling of AGN H13 was based on a smooth outflow.  It should reference the introduction where we say there are observational indications of clumping.  Should say that the difficulty associated with clumping is two fold - computationally implies many grid cells,  many potential parameters.  Ultimately, say that for simplicity we use microclumping.  Then say this has been used elsewhere, but with more recent references than what we have.  The point is we need to motivate micro clumping before we say what it means to the code  It's all mixed up as written. Aside: at some point this paper should mention H14, and hydro models more generally}
...
\caption
{
A cartoon showing the broad classes of sightlines 
in our model, illustrates how geometric effects lead to 
...
the output spectra in figure~\ref{fig:sightline}.  {\bf - Need to explain the source of the BAL composites}
...
\cite{reichard2003}, normalised to the flux at 2000\AA\ in each panel. Our simulated spectra do not contain
the semi-forbidden 
intercombination lines seen in quasar spectra because our radiative transfer code 
currently does not have a treatment for semi-forbidden lines.  {\bf Isn't this just a statement about the fact that we have not implemented CIII as a full macro-atom; if so we should say that}
This is especially noticeable with the strong C~\textsc{iii}]~1909\AA\, 
...
to become important coolants in the outer portion of the wind. {\bf we have gone from their being lines we should see, to important coolants.  I think we should stick to the fact that we would think our code would see them if we put C in as a macro atom.  Also I think this who discussion of CIII should be after we have discussed what looks right with the emission lines.}
...
\citep[e.g.][]{reichard2003}.  {\bf I think we mean EW here; otherwise I don't understand what we are talking about.  We could say ``line shapes and equivalent widths''.  I also think this requries more than one sentence of introduction.}
...
This supports the findings of \cite{risaliti2011}, who find that EW distributions 
in quasars are consistent with anisotropic emission from optically thick, 
disc-like structures for {\em both} the continuum source and BLR. 
We will explore these ideas further in a future study.  {\bf Wait!  All this says is that there has to be an alternative explanation.  It is not support for any specific explanation!. This last few sentences should be a separate paragraph.  They should start out: An alernative explanation proposed by risalite is that...}
...
sample of BALQSOs. The dotted lines show the best fit relations for non-BALQSOs 
