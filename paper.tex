...
{\bf 
Adiabatic cooling in the cell is calculated using the estimator
\begin{equation}
C_{ad} = f_V n_e k_B T_e V (\Delta \cdot v),
\end{equation}
where $V$ is the cell volume, $T_e$ is the electron temperature,
$n_e$ is the electron density and $\Delta \cdot v$ is the divergence of the 
velocity field at the centre of the cell.
For self-consistency, $k$-packets then have a finite 
destruction probability of 
\begin{equation}
P_{k,destruct} = \frac{C_{ad}}{C_{tot}},
\end{equation}
where $C_{tot}$ is the sum over all cooling rates in the cell.
We find that adiabatic cooling is insignificant in much of
the outflow.
...
{\bf This extreme $89^\circ$ viewing angle cannot represent a typical quasar within a unified model, as
extreme inclinations should be extremely underrepresented in quasar samples due to the
compton-thick absorber or `Torus' expected at high inclinations ().}
However, this spectrum does show that a model such as this can 
...
{\bf We would like to thank the anonymous referee for an insightful
