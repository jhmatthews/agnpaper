...
Monte Carlo radiative transfer and photoionization code PYTHON.  
Previously, we have shown that we could produce synthetic spectra 
resembling those of observed broad absorption line (BAL) quasars 
for plausible wind models, but only if the X-ray luminosity was limited to about  
10$^{43}$~erg~s$^{-1}$. Here, we investigate the  degree to which
clumping of the outflow allows us to 
produce synthetic spectra in the rest-frame ultraviolet (UV) 
that have the characteristics of quasars with X-ray luminosities as 
large as 10$^{45}$ erg s$^{-1}$. 
We conclude, perhaps not surprisingly, that clumping does maintain the ionization 
state of the wind necessary for strong BAL features in the rest-frame UV at more 
realistic X-ray luminosities. We examine the X-ray properties of our fiducial clumped model
...
excited line emission in, e.g., \la and \civ\, to emerge at the low inclination, 
`Type 1 quasar-like' angles. At the highest inclinations, the synthetic spectra 
possess prominent \mg\ and \al\ BALs, the absorption features seen in LoBAL quasars. 
Despite these successes, we are unable to reproduce all the emission lines seen in
quasar spectra with the correct equivalent width 
ratios in any of the models in our initial simulation grid,
and we find an angular dependence of emission line equivalent width despite the 
remarkable similarities in the observed spectral properties of BAL and non-BAL quasars.
Overall, our work suggests that BAL outflows can produce significant broad emission
lines and that accretion disc winds naturally reproduce the range of
ionization states seen in AGN, but a full quantitative agreement 
with quasar and BALQSO properties is not forthcoming.
...
% Overall, our work suggests that BAL outflows can produce significant broad emission
% lines and that accretion disc winds naturally reproduce the range of
% ionization states seen in AGN.
% This suggests that geometric unification
% involving an accretion disc wind is a promising scenario, but our results 
% pose a specific challenge to models in which an equatorial outflow
...
in the outflow. 
% Indeed, \cite{elitzur2014} show that a disc-wind BLR scenario
...
% according to radiative equilibrium, which assumes both statistical equilibrium 
% and that radiative heating balances radiative cooling.
...
% is that it moderates the ionization state and temperature structure of the plasma
...
% Arriving at a next-generation model}
...
% \begin{itemize}
% \item Do the synthetic spectra exhibit UV absorption lines 
% with $BI > 0$ at $\sim20\%$ of viewing angles (e.g. Knigge et al. 2010)?
% \item Do the synthetic spectra show  line emission emerge at low inclinations, with $EW\sim40\AA$ in \civ \citep[e.g. ][]{shen2011}?
% \item Do H recombination lines appear in the spectra, $EW\sim50\AA$ in \la 
% \citep[e.g. ][]{shen2011}?
% \item Do the spectra produce \mg\ and \al\ (LoBAL) absorption 
% features with $BI > 0$ at a small subset of BAL angles?
...
% We also examine trends with inclination in the synthetic spectra, 
% both in terms of the range of ionization states of the absorption lines and equivalent widths of the emission lines.
...
% $\log L$[H~$\alpha$] & $43.46$ & $42.86$ & ?? \\
% $EW$[C~\textsc{iv}] (erg~s$^{-1}$) & $3.96 \times 10^{44}$ & $2.62 \times 10^{44}$  \\
% $EW$[Mg~\textsc{ii}] (erg~s$^{-1}$) & $3.50 \times 10^{43}$ & $8.32 \times 10^{43}$ \\
% $EW$[H~$\alpha$] (erg~s$^{-1}$) & $3.50 \times 10^{43}$ & $8.32 \times 10^{43}$ \\
% $\log L_{1350}$ (erg~s$^{-1}$~Hz$^{-1}$) & $46.42$ & $45.56$ & $46.01 \pm 0.30$ \\
% $\log L_{3000}$ (erg~s$^{-1}$~Hz$^{-1}$) & $46.18$ & $45.36$ & $45.79 \pm 0.30$ \\
% $L_{bol})$ (erg~s$^{-1}$) & 2.62
% % $L_{1300})$(C\textsc{iv}) & 
% \\
% \multicolumn{3}{|l|}{At $73^\circ$ (`BALQSO-like')} \\  
% $\log L$[C~\textsc{iv}] (erg~s$^{-1}$) & $44.14$ & ??  \\
% $\log L$[Mg~\textsc{ii}] (erg~s$^{-1}$) & $42.82$ & $44.41$  \\
% $\log L$[H~$\alpha$] (erg~s$^{-1}$) & $42.86$ & ?? \\
% % $EW$[C~\textsc{iv}] (erg~s$^{-1}$) & $3.96 \times 10^{44}$ & $2.62 \times 10^{44}$  \\
% % $EW$[Mg~\textsc{ii}] (erg~s$^{-1}$) & $3.50 \times 10^{43}$ & $8.32 \times 10^{43}$ \\
% % $EW$[H~$\alpha$] (erg~s$^{-1}$) & $3.50 \times 10^{43}$ & $8.32 \times 10^{43}$ \\
% $\log L_{1350}$ (erg~s$^{-1}$~Hz$^{-1}$) & $45.56$ & $46.00$ \\
% $\log L_{3000}$ (erg~s$^{-1}$~Hz$^{-1}$) & $45.36$ & $45.79$ \\
\hline
\end{tabular}
\caption{
...
% This is because we do not fully solve the C level populations,
% and thus the level populations do not accurately reflect those
% as a full macro-atom with a 
...
% as well as a very weak \mg\ line. 
...
% However, the line-to-continuum ratios 
...
% seen in BALQSO spectra and composites. This suggests a fairly 
% fundamental problem with a geometric unification model in which 
% The line equivalent widths at high inclinations are
...
% JM XXX -- only include this if it's true!
...
% ionization structure, which can change the absorption 
% characteristics and resultant luminosities.
...
% \begin{figure}
% \centering
% \includegraphics[width=0.49\textwidth]{figures/hist_ew.png}
% \caption
% {
...
% % \begin{figure}
% % \centering
% % \includegraphics[width=0.45\textwidth]{figures/ew.png}
% % \caption
% % {
% % $W_{\lambda,RW}$ as a function of inclination in our example model (red line).
% % We also the mean $1/2$ equivalent width from DiPompeo et al. (2012) as a horizontal
% % line, with the shaded area corresponding to the standard deviation.
% % }
% % \label{fig:ew}
% % \end{figure}
