...
%DIF DEL paper.tex     Tue Nov 10 16:44:38 2015
%DIF ADD compare.tex   Tue Nov 10 16:44:34 2015
...
%\usepackage{pngfig}
%DIF 21c21
%DIF < %\usepackage[dvips]{color}mak
%DIF -------
%\usepackage[dvips]{color} %DIF > 
%DIF -------
%\usepackage{aabib}
...
\def\rmxaa{RMXAA}
\def\aaps{A\&As}
\DIFdelbegin %DIFDELCMD < \def\LA{Lyman\thinspace$\alpha$}
%DIFDELCMD < %%%
\DIFdelend 

\DIFdelbegin %DIFDELCMD < \newcommand{\EXPN}[2]{\mbox{$#1\times 10^{#2}$}}
%DIFDELCMD < \newcommand{\EXPU}[3]{\mbox{\rm $#1 \times 10^{#2} \rm\:#3$}}  %%%
%DIF <  exponent with units
%DIFDELCMD < \newcommand{\POW}[2]{\mbox{$\rm10^{#1}\rm\:#2$}}
%DIFDELCMD < \def\LUM{\:{\rm ergs\:s^{-1}}}
%DIFDELCMD < \def\FLUX{\:{\rm ergs\:cm^{-2}\:s^{-1}}}
%DIFDELCMD < \def\OIGS{\:{\rm ergs\:cm^{-2}\:s^{-1}\:\AA^{-1}}}
%DIFDELCMD < 
...
\DIFdelbegin \DIFdel{Testing Quasar Unification with Clumpy Wind Models
%DIF < Unifying quasars with clumpy wind models
}\DIFdelend \DIFaddbegin \DIFadd{Unifying quasars with clumpy wind models
}\DIFaddend }
...
\DIFaddend %\author[Matthews et al.]
...
Various unification schemes \DIFdelbegin \DIFdel{have been proposed to interpret the complex phenomenology 
of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend quasars and luminous active galactic nuclei (AGN) 
\DIFdelbegin \DIFdel{in terms of a simple axisymmetric 
picture involving a central black hole, }\DIFdelend \DIFaddbegin \DIFadd{propose that much of their complex phenomenology can be explained by a simple 
geometrical picture involving }\DIFaddend an accretion disc and \DIFdelbegin \DIFdel{an }\DIFdelend associated outflow.
Here, we \DIFdelbegin \DIFdel{continue our tests of  }\DIFdelend \DIFaddbegin \DIFadd{test }\DIFaddend this paradigm by \DIFdelbegin \DIFdel{comparing ``average'' AGN spectra to synthetic spectra of }\DIFdelend \DIFaddbegin \DIFadd{utilising our state-of-the-art radiative transfer 
and photoionization code to produce synthetic spectra from }\DIFaddend simple biconical disc wind models\DIFdelbegin \DIFdel{, using our state-of-the-art 
Monte Carlo radiative transfer and photoionization code}\DIFdelend . 
In particular, we \DIFdelbegin \DIFdel{investigate whether clumping of the outflow allows us to 
produce synthetic spectra in the rest-frame UV that
have the characteristics of AGN, 
assuming total luminosities as high as 10$^{45}$ ergs s$^{-1}$}\DIFdelend \DIFaddbegin \DIFadd{expand on our previous work by presenting a model that
exhibits many of the spectral features expected from a quasar unification model}\DIFaddend .
We find that a simple treatment of clumping (`microclumping') 
allows for a more realistic \DIFdelbegin \DIFdel{AGN luminosities}\DIFdelend \DIFaddbegin \DIFadd{X-ray luminosity in the model}\DIFaddend ,
while maintaining the ionization state necessary for strong BAL features\DIFdelbegin \DIFdel{in the rest-frame UV}\DIFdelend . 
We examine the X-ray properties of \DIFdelbegin \DIFdel{these simple clumped models
}\DIFdelend \DIFaddbegin \DIFadd{this new model
}\DIFaddend and find good agreement with existing X-ray samples of AGN and quasars.
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{We find that the }\DIFaddend dense, X-ray heated wind 
produces strong recombination and collisionally excited line emission in, 
e.g., \civ\ and \la, to emerge at the low inclination, `Type 1 quasar-like' angles. 
\DIFdelbegin \DIFdel{At the highest inclinations, the synthetic spectra }\DIFdelend \DIFaddbegin \DIFadd{The highest inclination models }\DIFaddend possess prominent \mg\ and \al\ BALs, the absorption 
features seen in LoBAL quasars. Despite these successes, we are unable to reproduce 
...
This is due to a fundamental constraint arising
from the anisotropy of emission from a classical thin disc. 
\DIFaddbegin \DIFadd{We briefly explore the impact of general relativistic effects
...
}\DIFaddend Overall, our work suggests that geometric unification
involving an accretion disc wind is a promising scenario, but our results 
pose a number of difficult challenges to such a model.
...
The BBB is often attributed to \DIFdelbegin \DIFdel{emission }\DIFdelend \DIFaddbegin \DIFadd{emssion }\DIFaddend from a 
...
In addition to the \DIFdelbegin %DIFDELCMD < {\em %%%
\DIFdel{inflowing }%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{inflowing }\DIFaddend accreting material,
\DIFdelbegin %DIFDELCMD < {\em %%%
\DIFdel{outflows }%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{outflows }\DIFaddend are ubiquitous in AGN
...
\DIFdelbegin \DIFdel{Perhaps the clearest evidence of outflows in AGN is  
the }\DIFdelend \DIFaddbegin \DIFadd{Approximately $20\%$ of quasars exhibit }\DIFaddend blueshifted ($\sim 0.1c$)
broad absorption lines (BALs) in the ultraviolet\DIFdelbegin \DIFdel{seen in approximately $20\%$ of quasars
}\DIFdelend \DIFaddbegin \DIFadd{,
...
BAL quasars (BALQSOs) is in terms of an accretion disc wind \DIFdelbegin \DIFdel{. 
According to this }\DIFdelend \DIFaddbegin \DIFadd{(ADW). 
Within the ADW }\DIFaddend paradigm, a biconical wind rises from 
the accretion disc and the BALQSO fraction is associated with
the covering factor of the outflow. 
\DIFdelbegin \DIFdel{Polarisation studies expect the wind }\DIFdelend \DIFaddbegin \DIFadd{The wind is expected }\DIFaddend to be roughly equatorial \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{Due to their ubiquitous nature,
disc winds }\DIFdelend \DIFaddbegin \DIFadd{from polarisation results
\mbox{%DIFAUXCMD
\citep{goodrich1995, cohen1995}
}%DIFAUXCMD
.
ADWs }\DIFaddend offer a natural explanation for the
...
Within this \DIFdelbegin \DIFdel{unification }\DIFdelend framework, the broad-line region (BLR) can 
correspond either to the dense wind base or \DIFaddbegin \DIFadd{dense }\DIFaddend clumps embedded
in the \DIFdelbegin \DIFdel{outflow}\DIFdelend \DIFaddbegin \DIFadd{outflows}\DIFaddend . Indeed, \cite{elitzur2014} \DIFdelbegin \DIFdel{show }\DIFdelend \DIFaddbegin \DIFadd{found }\DIFaddend that a disc-wind BLR scenario
...
HiBALQSOs, which only exhibit \DIFdelbegin \DIFdel{high }\DIFdelend \DIFaddbegin \DIFadd{higher }\DIFaddend ionization line absorption; LoBALQSOs \DIFdelbegin \DIFdel{, }\DIFdelend which also show
absorption in lower ionization state species such as \mg\ and \al; and
FeLoBALQSOs \DIFdelbegin \DIFdel{, }\DIFdelend which show further absorption in Fe~\textsc{ii} and \textsc{iii}.
In unified \DIFdelbegin \DIFdel{geometric }\DIFdelend models, this is generally attributed to ionization stratification
of the outflow \citep[e.g.][]{elvis2000}.

% As well as acting as a source of photoionized plasma, a 
%DIF <  As well as imprinting clear line absorption and emission
...
\DIFaddbegin \DIFadd{As well as imprinting clear line absorption and emission
features, disc winds may also have a profound effect on the structure and 
emergent }{\em \DIFadd{continuum}} \DIFadd{of the accretion disc itself.
...
Despite the clear importance of \DIFdelbegin \DIFdel{disc winds in shaping quasar and AGN spectra}\DIFdelend \DIFaddbegin \DIFadd{ADWs in understanding AGN SEDs and accretion physics}\DIFaddend , 
much of the underlying outflow physics remains highly uncertain. 
Several possible driving mechanisms \DIFaddbegin \DIFadd{for ADWs }\DIFaddend have been proposed, including
...
%DIF <  The presence of line-locked features \citep{bowler2014} 
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{The presence of line-locked features \mbox{%DIFAUXCMD
\citep{bowler2014}
}%DIFAUXCMD
and the `ghost of }\la\DIFadd{' (Arav et al. 1996; Arav 1996; North 2006; but see 
also Cottis et al. 2010) }\mbox{%DIFAUXCMD
\nocite{arav1995, arav1996, north2006,cottis2010}
}%DIFAUXCMD
\DIFadd{in the spectra of some BALQSOs also gives clearer evidence that line-driving is
at least partially contributing to the acceleration of the wind.
}

\DIFadd{The }\DIFaddend efficiency of line-driving is crucially dependent on the ionization state 
...
%DIF <  Hydrodynamic simulations of line-driven disc winds also found a shielding region
...
\DIFaddbegin \DIFadd{Hydrodynamic simulations of line-driven disc winds also found a shielding region
...
}\DIFaddend An additional or alternative solution is that the wind is clumped \citep[e.g.][]{hamann2013}
...
\DIFaddbegin \DIFadd{on a variety of scales }\DIFaddend are also produced in simulations of line-driven 
outflows in AGN \DIFdelbegin \DIFdel{, although on very different scales to line-driven instabilities 
\mbox{%DIFAUXCMD
\citep{PSK2000,PK04,progakurosawa2010,proga2014}
}%DIFAUXCMD
.
Nevertheless, clumpy winds }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{PSK2000,PK04,progakurosawa2010,proga2014}
}%DIFAUXCMD
.
Clumpy winds therefore }\DIFaddend offer an observationally motivated and theoretically 
...
\DIFdelbegin \DIFdel{We have been engaged in a project to determine whether it is possible to simulate the properties of the spectra of AGN, including BAL QSOs, }\DIFdelend \DIFaddbegin \DIFadd{There has been some success }\DIFaddend using simple
kinematic prescriptions for biconical disc winds \DIFdelbegin \DIFdel{using a Monte Carlo radiative transfer (MCRT) code that calculates the ionization structure of the wind and simulates the spectra from such a system \mbox{%DIFAUXCMD
\citep[][hereafter H13]{simlong2008,sim2010,higginbottom2013}
}%DIFAUXCMD
. 
The results have encouraging in the sense that in }\DIFdelend \DIFaddbegin \DIFadd{to model 
AGN and quasar outflows \mbox{%DIFAUXCMD
\citep[][hereafter H13]{simlong2008,sim2010,higginbottom2013}
}%DIFAUXCMD
. 
}\DIFaddend H13 \DIFdelbegin \DIFdel{, we showed we could produce simulated spectra that resembled that of BAL QSOs, as long as the luminosity of the central engine was relatively low, of order }%DIFDELCMD < \POW{43}{\LUM} %%%
\DIFdel{and the mass loss rate was relatively high, of order the mass accretion rate.
However, at higher luminosities, the wind was so ionized that UV absorption lines were not produced.  In addition, and in part due to limitations in our radiative transfer code, the }\DIFdelend \DIFaddbegin \DIFadd{successfully produced a benchmark model for BALQSOs. However, the model
...
Second, the }\DIFaddend model failed to produce \DIFdelbegin \DIFdel{spectra with }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend strong emission lines 
\DIFdelbegin \DIFdel{at any inclination angle.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Here }\DIFdelend \DIFaddbegin \DIFadd{required at low inclinations in a unified model.
In this paper, }\DIFaddend we attempt to address \DIFdelbegin \DIFdel{both of }\DIFdelend these issues, \DIFdelbegin \DIFdel{by introducing clumping into our model and a more complete treatment of H and He into our radiative transfer }\DIFdelend \DIFaddbegin \DIFadd{and test the disc wind 
unification model using Monte Carlo radiative transfer (MCRT) and photoionization 
}\DIFaddend calculations. The \DIFdelbegin \DIFdel{remainder of this paper is organized as follows:
...
}\DIFdelend \DIFaddbegin \DIFadd{paper is organised as follows.
}\DIFaddend In section 2, we describe some of the important photoionization 
and MCRT aspects of the code. \DIFdelbegin \DIFdel{We then }\DIFdelend \DIFaddbegin \DIFadd{In section 3, we }\DIFaddend outline the model\DIFdelbegin \DIFdel{in section 3}\DIFdelend , including 
a description of our clumping implementation\DIFdelbegin \DIFdel{and success criteria. Section }\DIFdelend \DIFaddbegin \DIFadd{. In section }\DIFaddend 4\DIFdelbegin \DIFdel{contains }\DIFdelend \DIFaddbegin \DIFadd{, we present }\DIFaddend the results 
from a clumped model\DIFdelbegin \DIFdel{, which we discuss results
in comparison to observational data. Finally}\DIFdelend \DIFaddbegin \DIFadd{. In section 5 we discuss our results, 
focussing particular on the anisotropy of 
disc emission and GR effects, and finally, in section 6}\DIFaddend , we summarise our findings\DIFdelbegin \DIFdel{in section 5.
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend 
...
\DIFdelbegin \DIFdel{For this study, we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend use the MCRT code \py\ \DIFdelbegin \DIFdel{we have developed }\DIFdelend to carry out our radiative transfer and photoionization
\DIFdelbegin \DIFdel{simulations }\DIFdelend \DIFaddbegin \DIFadd{calculations }\DIFaddend in non-local thermodynamic equilibrium (non-LTE). 
The code \DIFdelbegin \DIFdel{can be used to model a variety of
disc-wind systems; it }\DIFdelend has been used \DIFdelbegin \DIFdel{with application to }\DIFdelend \DIFaddbegin \DIFadd{to model }\DIFaddend accreting white dwarfs 
...
and quasars \DIFdelbegin \DIFdel{/AGN }\DIFdelend (H13\DIFdelbegin \DIFdel{, H14}\DIFdelend ). \nocite{noebauer, M15, LK02}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The basic premise involves generating photons from an accretion disc
...
Once these ``ionization cycles'' are complete, 
then a series of further iterations
...
We focus here on the specific changes made to improve the }\DIFdelend \DIFaddbegin \DIFadd{These studies contain extensive detail on the code, 
so we only briefly describe the key elements of the global 
}\DIFaddend ionization calculation and \DIFdelbegin \DIFdel{to allow for clumping in the wind.
...
}\DIFdelend \DIFaddbegin \DIFadd{other important aspects.
}\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdelend \subsection{Line transfer}

\DIFdelbegin \DIFdel{Our approach to line transferis based upon the macro-atom implementation developed by \mbox{%DIFAUXCMD
\cite{lucy2002, lucy2003}
}%DIFAUXCMD
. In our case, for reasons of computational effieciency, }\DIFdelend \DIFaddbegin \DIFadd{To treat line transfer, }\DIFaddend we adopt the \DIFdelbegin \DIFdel{hybrid macro-atom }\DIFdelend \DIFaddbegin \DIFadd{same hybrid }\DIFaddend scheme 
described by M15%DIF < (an improvement over that used by H14 for our earlier quasar study), 
\DIFaddbegin \DIFadd{, 
}\DIFaddend in which the energy flows
...
or full `macro-atoms'. \DIFaddbegin \DIFadd{The macro-atom implementation 
is described in full by \mbox{%DIFAUXCMD
\cite{lucy2002, lucy2003}
}%DIFAUXCMD
.
}\DIFaddend Our scheme allows one to treat non-LTE line transfer in radiative equilibrium
without approximation for elements \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{which }\DIFaddend are identified as 
...
in determining \DIFdelbegin \DIFdel{their }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend level populations and resultant line emission\DIFdelbegin \DIFdel{, and because we were especially interested in the contribution to AGN spectra of }%DIFDELCMD < \LA%%%
\DIFdel{.
In H13, all atoms were treated in a two-level approximation.
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend 
...
MC rate estimators as described by \DIFdelbegin \DIFdel{Lucy (2002,2003)}\DIFdelend \DIFaddbegin \DIFadd{M15}\DIFaddend . Previously (LK02, H13, M15),
we \DIFdelbegin \DIFdel{used }\DIFdelend \DIFaddbegin \DIFadd{adopted }\DIFaddend a modified Saha approach to calculate the ionization fractions
of simple-ions. \DIFdelbegin \DIFdel{As part of  this effort, we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend have now improved \DIFdelbegin %DIFDELCMD < {\sc %%%
\DIFdel{PYTHON}%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{our code }\DIFaddend to explicitly solve the 
...
processes \DIFdelbegin \DIFdel{that would previously have necessitated 
approximate treatments}\DIFdelend \DIFaddbegin \DIFadd{such as Auger ionization}\DIFaddend .

\DIFdelbegin \DIFdel{In order to calculate the photoionization rate, 
we }\DIFdelend %DIF > To avoid approximating the complex AGN SED as e.g. a dilute blackbody \citep[][M15]{ML93},
\DIFaddbegin \DIFadd{We }\DIFaddend model the SED in a \DIFdelbegin \DIFdel{grid }\DIFdelend cell using the technique described by H13. In this scheme,
the mean intensity \DIFdelbegin \DIFdel{, $J_{\nu}$ }\DIFdelend in a series of $n$ bands is modeled as \DIFdelbegin \DIFdel{either a power law or exponential
}\DIFdelend \DIFaddbegin \DIFadd{a normalised power law }\DIFaddend in 
frequency $\nu$
\DIFaddbegin \begin{equation}\DIFadd{
J_{\nu,i}=K_{pl}\nu^{\alpha_{pl}},
}\end{equation}
\DIFadd{for a band $i$, or an exponential 
}\begin{equation}\DIFadd{
J_{\nu,i}=K_{exp} e^{(-h\nu/k T_{exp})}.
}\end{equation}
\DIFadd{Here, $K_{exp}$}\DIFaddend , \DIFdelbegin \DIFdel{with the }\DIFdelend \DIFaddbegin \DIFadd{$K_{pl}$, $T_{exp}$ and $\alpha_{pl}$ are spectral }\DIFaddend fit parameters
deduced from \DIFaddbegin \DIFadd{the }\DIFaddend band-limited radiation field estimators.
%DIF <  \begin{equation}
%DIF <  J_{\nu,i}=K_{pl}\nu^{\alpha_{pl}},
%DIF <  \end{equation}
...
where $\sigma_j$ is the photoionization cross-section \DIFdelbegin \DIFdel{, $n_e$
is the electron density }\DIFdelend and $C_{j}$
represents the collisional ionization coefficient.
The recombination rate {\em into} ion $j$ is given by 
\begin{equation}
R_{j+1,j}(T_e) = (\alpha^j_{RR} + \alpha^j\DIFaddbegin \DIFadd{_{DR} + \alpha^j}\DIFaddend _{CR}) n_{j+1} n_e,
\end{equation}
where each $\alpha^j$ here is the recombination rate coefficient into the ground state of ion $j$.
The subscripts denote radiative\DIFaddbegin \DIFadd{, dielectronic }\DIFaddend and 
collisional (three-body) recombination, respectively.
\DIFdelbegin \DIFdel{$T_e$ is the electron temperature. 
%DIF < % We neglect recombination to and from excited states in the simple-ion calculation. 
}\DIFdelend \DIFaddbegin \DIFadd{We neglect recombination to and from excited states in the simple-ion calculation. 
...
\DIFdelbegin \DIFdel{We stress that this approximation is not required for ions
...
\DIFdelbegin \subsection{\DIFdel{The Simulation Grid: Arriving at a next-generation model}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend %DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\DIFdelbegin \DIFdel{Using this prescription, we conducted a limited parameter
}\DIFdelend %DIF >  RESULTS
\DIFaddbegin 
...
\DIFadd{Here we describe the results from our model, the parameters
of which are shown in table~1.
Parameters differing from the benchmark model of
H13 are highlighted with an asterisk.
This set of parameters
was arrived at by conducting a limited grid }\DIFaddend search over a 
5-dimensional parameter space involving the variables
$r_{min}$, $\theta_{min}$, $f$, $\alpha$ and \DIFdelbegin \DIFdel{$R_v$.
The grid points are shown in table 1.
The aim here was to first fix $M_{BH}$ and $\dot{m}$ to their H13 values, and increase $L_X$ to $10^{45}$~erg~s$^{-1}$ (a more realistic value for a 
quasar of $10^9M_\odot$ and an eddington fraction of $0.2$; see section~\ref{sec:xray}).
We then }\DIFdelend \DIFaddbegin \DIFadd{$R_V$.
The full grid, including output spectral files and plots can be found at
}\url{jhmatthews.github.io/quasar-wind-grid/}\DIFadd{.
We }\DIFaddend evaluated these models \DIFaddbegin \DIFadd{qualitatively }\DIFaddend based on the following
criteria:
\begin{itemize}
\item Does the model \DIFdelbegin \DIFdel{avoid over-ionization and thus }\DIFdelend \DIFaddbegin \DIFadd{maintain the right ionization state to }\DIFaddend produce strong BALs\DIFdelbegin \DIFdel{with $BI > ??$}\DIFdelend ?
\item Does significant line emission emerge at low inclinations\DIFdelbegin \DIFdel{, with $EW\sim50\AA$ in }%DIFDELCMD < \civ%%%
\DIFdelend ?
\item Do H recombination lines appear in the spectrum\DIFdelbegin \DIFdel{, $EW\sim50\AA$ in }%DIFDELCMD < \la%%%
\DIFdelend ?
...
\end{itemize}
\DIFdelbegin \DIFdel{We arrived at a model
}\DIFdelend In this section, we present one of the most promising models and discuss
...
\DIFdelbegin \DIFdel{The full grid, including output synthetic spectra and plots can be found at
...
%DIFDELCMD < \begin{tabular}{p{2cm}p{1cm}p{1cm}p{1cm}p{1cm}}
%DIFDELCMD < %%%
\DIFdel{Parameter }%DIFDELCMD < & \multicolumn{4}{|l|}{Grid Point Values}  \\
%DIFDELCMD < \hline \hline 
%DIFDELCMD < %%%
\DIFdel{$r_{min}$ 	}%DIFDELCMD < &	 %%%
\DIFdel{$6r_{g}$ }%DIFDELCMD < & %%%
\DIFdel{$60r_{g}$ }%DIFDELCMD < & \multicolumn{2}{|l|}{$300r_{g}$} \\ 
%DIFDELCMD < %%%
\DIFdel{$\theta_{min}$ 	}%DIFDELCMD < &	 %%%
\DIFdel{$40^{\circ}$ }%DIFDELCMD < & %%%
\DIFdel{$55^{\circ}$ }%DIFDELCMD < & \multicolumn{2}{|l|}{$70^{\circ}$} \\ 
%DIFDELCMD < %%%
\DIFdel{$R_v$  	        }%DIFDELCMD < &	 %%%
\DIFdel{$10^{18}$cm }%DIFDELCMD < & \multicolumn{3}{|l|}{$10^{19}$cm} \\ 
%DIFDELCMD < %%%
\DIFdel{$\alpha$ 	}%DIFDELCMD < &	 %%%
\DIFdel{$0.5$ }%DIFDELCMD < & %%%
\DIFdel{$0.6$ }%DIFDELCMD < & %%%
\DIFdel{$0.75$ }%DIFDELCMD < & %%%
\DIFdel{$1.5$ }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{$f$ 	}%DIFDELCMD < &	 %%%
\DIFdel{$0.01$ }%DIFDELCMD < & %%%
\DIFdel{$0.1$ }%DIFDELCMD < & \multicolumn{2}{|l|}{$1$}  \\
%DIFDELCMD < \hline 
...
%DIFDELCMD < %%%
\DIFdelend \begin{figure*} %fullpage
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1.0\textwidth]{figures/link8.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.0\textwidth]{figures/link.png}
\DIFaddendFL \caption
{
\DIFdelbeginFL \DIFdelFL{Contour plots showing the logarithm of some important 
physical }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Physical }\DIFaddendFL properties of the outflow\DIFdelbeginFL \DIFdelFL{. Symbols are defined in the text}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, shown by the coloured contours}\DIFaddendFL .
...
\DIFdelbeginFL \DIFdelFL{The line luminosities represent the luminosity of photons
...
$\alpha$ 	&	 $0.6^*$ \\
$f$ 	&	 $0.01^*$  \\
\DIFdelbeginFL \DIFdelFL{$n_x$ 	}%DIFDELCMD < &	 %%%
\DIFdelFL{$100$  }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{$n_z$ 	}%DIFDELCMD < &	 %%%
...
% \begin{figure*} %fullpage
% \centering
%DIF <  \includegraphics[width=1.0\textwidth]{figures/0946.png}
%DIF >  \includegraphics[width=1.0\textwidth]{figures/opt.png}
% \caption
% {
%DIF <  Synthetic spectra at $72^\circ$ compared to an HST STIS spectrum
%DIF <  of PG0946+301 (Arav et al. 2000). The spectrum is scaled to the flux at 1300\AA\
%DIF <  for direct comparison. 
...
\DIFdel{(SDSS) }\DIFdelend \DIFaddbegin \DIFadd{We also show a comparison to composite }\DIFaddend quasar and BALQSO \DIFdelbegin \DIFdel{composites from \mbox{%DIFAUXCMD
\cite{reichard2003}
}%DIFAUXCMD
, normalised to the flux at 2000\AA}%DIFDELCMD < \ %%%
\DIFdel{in each panel}\DIFdelend \DIFaddbegin \DIFadd{spectra}\DIFaddend .
We show a cartoon illustrating how geometric effects determine
the output spectra in figure~\ref{fig:sightline}.

\subsubsection{Broad absorption lines\DIFdelbegin \DIFdel{(`BALQSO-like' angles)}\DIFdelend }
...
cooler, \DIFdelbegin \DIFdel{low }\DIFdelend \DIFaddbegin \DIFadd{lower }\DIFaddend ionization material at the base of the wind
starts to intersect the line of sight. This produces 
multiple absorption lines in \DIFaddbegin \DIFadd{lower ionization }\DIFaddend species such as \mg,
...
At \DIFdelbegin \DIFdel{the lowest inclination BAL sightlines}\DIFdelend \DIFaddbegin \DIFadd{lower inclinations}\DIFaddend , the strongest absorption occurs at the red edge,
whereas at \DIFdelbegin \DIFdel{higher }\DIFdelend \DIFaddbegin \DIFadd{high }\DIFaddend inclinations (and for the strongest BALs)
...
line shapes (see e.g. Trump et al. 2006; Knigge et al 2008\DIFdelbegin \DIFdel{, Filiz Ak et al. 2014}\DIFdelend ).
...
meaning that any scattered \DIFdelbegin \DIFdel{component to the BAL troughs is absorbed by line opacity}\DIFdelend \DIFaddbegin \DIFadd{contribution is obscured by the saturated troughs}\DIFaddend .
...
\subsubsection{Broad emission lines\DIFdelbegin \DIFdel{(`quasar-like' angles)}\DIFdelend }

\DIFdelbegin %DIFDELCMD < \begin{figure*} %%%
%DIF < fullpage
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=1.0\textwidth]{figures/ew.png}
%DIFDELCMD < \caption
%DIFDELCMD < {
%DIFDELCMD < %%%
\DIFdel{Physical properties of the outflow, shown by the coloured contours.
The solid black line marks a sphere at $1000~r_G$.
The dotted lines show the $72^\circ$ and $78^\circ$ sightlines 
to the centre of the system, and illustrate that different sightlines
intersect material of different ionization states.
}%DIFDELCMD < }
%DIFDELCMD < \label{fig:ew}
%DIFDELCMD < \end{figure*} %%%
%DIF < fullpage
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We find significant collisionally excited line emission
emerges
}\DIFdelend \DIFaddbegin \DIFadd{We find that the model can produce significant line emission
}\DIFaddend at low inclinations\DIFdelbegin \DIFdel{in the synthetic spectra}\DIFdelend , particular in \DIFdelbegin \DIFdel{the }\DIFdelend \civ\DIFdelbegin %DIFDELCMD < \ %%%
\DIFdel{line.
The }\DIFdelend \DIFaddbegin \DIFadd{, 
and the }\DIFaddend improved treatment of recombination \DIFdelbegin \DIFdel{also }\DIFdelend results in a strong \la\
line. In the context of unification, this is a promising result, 
and shows that a biconical wind can produce significant 
emission at `quasar-like' angles. \DIFdelbegin \DIFdel{The spectrado not contain the }\DIFdelend \DIFaddbegin \DIFadd{To assess the ability of the model to match real 
quasar spectra, we also show }{\sl \DIFadd{Sloan Digital Sky Survey}} \DIFadd{(SDSS) quasar composites from
\mbox{%DIFAUXCMD
\cite{reichard2003}
}%DIFAUXCMD
, normalised to the flux at 2000\AA}\ \DIFadd{in each panel.
...
This is especially noticable with the }\DIFaddend strong C~\textsc{iii}]~1909\AA\, 
line \DIFdelbegin \DIFdel{seen }\DIFdelend in the quasar composite spectra.
\DIFdelbegin \DIFdel{This is because we do not yet treat C as a full macro-atom with a 
full collisional rates between forbidden
or semi-forbidden transitions, as would be required.
}\DIFdelend The critical density of the C~\textsc{iii}]~1909\AA\, line 
is $n_e\sim10^{9.5}~\rm{cm^{-3}}$\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{wei1988}
}%DIFAUXCMD
}\DIFdelend , which is higher than much of the 
outer portion of our wind. We therefore expect \DIFdelbegin \DIFdel{a model with these 
parameters to produce a C~}\textsc{\DIFdel{iii}}%DIFAUXCMD
%DIFDELCMD < ]%%%
\DIFdel{~1909\AA}%DIFDELCMD < \ %%%
\DIFdel{line
with a proper treament}\DIFdelend \DIFaddbegin \DIFadd{intercombination lines
to become important coolants in the outer portion of the wind}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < \label{angular}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{discsed}
\DIFaddend 

\DIFdelbegin \DIFdel{In order to quantitatively assess how emission lines change with 
inclination when blue-shifted absorption 
may affect the line profile, we define the `red wing equivalent width' ($W_{\lambda,RW}$) as }\begin{displaymath}\DIFdel{
W_{\lambda,RW} = \int_{\lambda_0}^{\lambda^\prime} \left( 1 - \frac{F_\lambda}{F_0} \right) d\lambda
\label{rwew}
}\end{displaymath}
%DIFAUXCMD
\DIFdel{where $F_0$ is the continuum flux and the integral is calculated from $\lambda_0$, 
line centre,
to a wavelength $\lambda^\prime$ where the flux has returned to the continuum level. This quantity is shown as a function of inclination in figure 5 for the }%DIFDELCMD < \civ%%%
\DIFdelend \DIFaddbegin \begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{figures/agnspec.png}
\caption
{
\DIFaddFL{$F_\lambda$ for three different wavelengths
as a function of inclination from }\agn\DIFaddendFL \ \DIFdelbeginFL \DIFdelFL{and }%DIFDELCMD < \mg%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{models, 
compared to a classical AD. The }\agn\DIFaddendFL \ \DIFdelbeginFL \DIFdelFL{UV lines.
We also plot show the $W_{\lambda,RW}$ expected from isotropic line emission and 
a foreshortened 
and limb darkened disc  as well as $1/2$ equivalent widths from \mbox{%DIFAUXCMD
\cite{dipompeo2012b}
}%DIFAUXCMD
. 
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{models are computed 
...
\DIFdelbegin \DIFdel{BALQSOs and quasars generally possess very similar emission line properties 
\mbox{%DIFAUXCMD
\citep[e.g.][]{weymann1991,reichard2003}
}%DIFAUXCMD
.
To demonstrate this, we show equivalent width
...
}\DIFdelend \DIFaddbegin \DIFadd{When comparing BALQSO and quasar composites, it is apparent
that they possess remarkably similar line strengths and widths 
\mbox{%DIFAUXCMD
\citep[e.g.][]{weymann1991,reichard2003}
}%DIFAUXCMD
.
}\DIFaddend This presents a challenge to our model, as well as the geometric 
unification picture in general.
\DIFdelbegin \DIFdel{One obvious potential solution to this problem is to hypothesize a more isotropicdistribution 
for the emergent condition than predicted by a }\DIFdelend \DIFaddbegin \DIFadd{Limb darkening and foreshortening causes the disc continuum emission to be 
...
significant fine-tuning when considering a }\DIFaddend classical thin disc. 
\DIFaddbegin \DIFadd{An alternative, simpler solution is that the emergent continuum is roughly isotropic.
}

\DIFaddend General relativistic effects -- specifically, light bending
and relativistic beaming -- can cause 
the accretion disc SED to become more isotropic \citep[e.g.][]{zhang1997,munozdarias2013}.
\DIFdelbegin \DIFdel{However, we have verified using AGNSPEC (REFs to Hubeny)
that this effect is small in the UV and optical wavelength regimes, 
and the disc is still anisotropic.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Reprocessing by an extended outflow may also cause a more isotropic
...
}\DIFdelend \DIFaddbegin \DIFadd{To generate GR disc spectra, we use the code }\agn\ \DIFadd{\mbox{%DIFAUXCMD
\citep{hubeny2000,davishubeny2006,davis2007}
}%DIFAUXCMD
. 
The output flux at three different wavelengths
as a function of inclination for an }\agn\ \DIFadd{model with the same disc and BH parameters
...
emit more isotropically. This supports the findings of \mbox{%DIFAUXCMD
\cite{risaliti2011}
}%DIFAUXCMD
, 
who }\DIFaddend find that EW distributions in quasars are consistent with anisotropic emission 
...
%DIFDELCMD < %%%
\DIFdel{It is also possible that the equatorial paradigm invoked from early polarisation
...
($\sim0$-$45^\circ$), or that }%DIFDELCMD < {\em %%%
\DIFdel{both}%DIFDELCMD < } %%%
\DIFdel{evolutionary and geometric explanations are required.
...
of BALQSOs}\DIFdelend \DIFaddbegin \DIFadd{We will explore these ideas further in a future study}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdelend % Figure 5 also shows the spectrum into the optical,
...
\DIFdelbegin %DIFDELCMD < \label{sec:xray}
%DIFDELCMD < %%%
\DIFdelend 

\begin{figure*} %fullpage
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1.0\textwidth]{figures/lx_a05_pre.png}
%DIFDELCMD < %%%
...
\DIFdelbegin \DIFdel{As observed in a BALQSO sample by \mbox{%DIFAUXCMD
\cite{filizak2014}
}%DIFAUXCMD
, we find that
...
%DIF <  \centering
%DIF <  \includegraphics[width=1.0\textwidth]{figures/adjusted_withbals.png}
%DIF <  \caption
%DIF <  {
...
%DIF <  \begin{figure}
%DIF <  \centering
%DIF <  \includegraphics[width=0.45\textwidth]{figures/f2000_m9.png}
%DIF <  \caption
%DIF <  {
...
%DIFDELCMD < {\bf %%%
\DIFdel{Note: One of the things I definitely wondered about is whether our "model" would produce the fraction of BALs seen in the sky}%DIFDELCMD < }
%DIFDELCMD < 
...
\item \DIFadd{We have examined the effect of GR on our disc SED, using the disc atmosphere
and GR ray-tracing code }\agn\DIFadd{. While including GR effects
