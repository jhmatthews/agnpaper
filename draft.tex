...
N.~Higginbottom$^1$, K.~S.~Long$^2$, S.~A.~Sim$^3$, S.~W.~Mangham$^1$.
\medskip  
...
\item Can an outflow also be the BLR? (unification)
\item some discussion of scales, referencing e.g. reverb maps, variability, microlensing, Arav 
\item AGN variability leading to clumping background: stellar winds, clumping in AGN winds.
\item Modelling efforts so far (Proga, Higginbottom, etc.)
\item Plan of paper
\end{enumerate}
%%\item 
...
% The scheme allows one to treat 

\subsection{Ionization Scheme}

Describe the ionization scheme. Combination of H13 and M15 but with rate equations
solved explicitly for ionization solution.
...
% and in principle allows the direct addition of extra physical processes such as Auger ionization,
% which would have to be approximated if using the previous technique.
...
\subsection{A Simple Biconical Disc Wind Model}
...
of $L_{X}1\times10^{43}~\rm{ergs~s^{-1}}$. Without clumping, this wind would be over-ionized 
to the extent that opacities in e.g., \civ\ would be entirely negligible (see H13).

One useful measure of the ionization state is the ionization parameter, $U$, given by
\begin{equation}
...
% In particular, we are left with the following possibilities:

% \begin{enumerate}
% \item BALQSOs are not, as a rule, drawn from the quasar population, and 
% instead represent a subclass of objects with strong outflows and differing
% emission line properties to normal luminous AGN and quasars.
% \item A combination of GR effects and reprocessing of the disc continuum
% by the outflow results in a more or less isotropic continuum, explaining
% the comparable line EWs between BALQSOs and quasars. 
% \item The line emission is anisotropic in such a way so as to counter the 
% anisotropy of the disc continuum.
% \end{enumerate}

% We favour a combination of the first two scenarios, based on a combination
% of physical and ontological arguments. It is clear from samples of BALQSOs
% and normal quasars that they have a fairly large range of Eddington fractions, 
% black hole masses and inclinations. The composite spectra will therefore
% be subject to a myriad of complex selection effects. 
