...
def plot_ew():
	...
	path = "/Users/jmatthews/Documents/runs/qso_he_test/with_estfix/"
	...
	specs.append("/Users/jmatthews/Documents/runs/qso_he_test/with_estfix/nextgen_a05_pre_he")
	#specs.append("/Users/jmatthews/Documents/runs/qso_clumped_fixgrid/stepgrid_from_a06/run8_mdotw10p0_lx1e46")
	
	print specs
	#figure(figsize=(14,6))
	figure(figsize=(7,11))
	label_subscript = "\lambda, RW"
	...
	for ifile in range(len(specs)):
		...
		labels=[r"$W_{%s}$~(C\textsc{iv}, Model)" % label_subscript,r"$W_{%s}$~(Mg\textsc{ii}, Model)" % label_subscript]
		#labels=[r"$W_{%s}$~(Mg\textsc{ii}, Model)" % label_subscript]
		...
		subplot(211)
		l_ha, ews_c4, l_c4, ews_mg2, l_mg2, bal_flag, r_flag = np.loadtxt("/Users/jmatthews/Documents/J_ApJS_194_45.tar/catalog.dat", unpack=True, usecols=(107,103,86,82, 13, 14))
		ews = ews_c4

		select1 = (ews>0) 
		select2 = (bal_flag == 0) 
		hist(0.5*ews[select1*select2],bins=bins, facecolor=colors[0], alpha=0.5, label="non-BALs", orientation ='horizontal', normed=True)
		
		select3 = (bal_flag >= 1) 
		hist(0.5*ews[select1*select3],bins=bins, facecolor=colors[2],  alpha=0.5, label="BALs", orientation ='horizontal', normed=True)

		#select3 = (bal_flag >= 1) 
		#hist(0.5*ews[select1*select3],bins=bins, facecolor=colors[3],  alpha=0.5, label="RL BALs", orientation ='horizontal', normed=True)

		#select3 = (bal_flag == 0) * (r_flag == 1)
		#hist(0.5*ews[select1*select3],bins=bins, facecolor=colors[4],  alpha=0.5, label="RL non-BALs", orientation ='horizontal', normed=True)

		#select3 = (bal_flag > 1) 
		#hist(0.5*ews_mg2[select1*select3],bins=bins, alpha=0.5, label="Lo3BALs", orientation ='horizontal', normed=True)

		select2 = (bal_flag == 1)
		xlabel("Normalised Counts", fontsize=20)
		ylabel(r"$W_{\lambda} / 2$~(\AA)", fontsize=20)
		ylim(0,140)
		float_legend()
		...
		for j in range(len(ws)):
			...
			fill_between(np.arange(70,83), 0,140, facecolor=c[1], alpha=0.5)
			...
			if ifile == len(specs) - 1:
				...
				if j == 0: ylabel(r"$W_{%s}$ (\AA)" % label_subscript, fontsize=20)
	...
	savefig("hist_ew.png", dpi=300, bbox_inches='tight')
	clf()
