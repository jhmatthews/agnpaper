...
fnames = [("../specs/fiducial_xray_fine", "../specs/h13.spec"), ("../specs/grid10/nextgen_a05_3_30", "../specs/grid10/nextgen_a05")]
...
scatter(10.0**l2500, alphaox, c="k", s=40)
scatter(10.0**dat2[0].astype(np.float), dat2[2].astype(np.float), s=40, edgecolors="k", facecolors="None")
scatter(10.0**bal_l2500, bal_alphaox, marker="^", edgecolors="#44AA99", facecolors="None", s=150, lw=2)

for jj in range(2):
	for i in range(len(angles[jj])):
		text(L_2500s[jj][i]-(L_2500s[jj][i]*0.13), alpha_oxs[jj][i]-0.03, angles[jj][i], color=colors[jj],fontsize=16, bbox=bbox_props[jj])

#scatter(L_2500s[0], alpha_oxs[0], marker="o", facecolors="None", edgecolors=colors[0], s=200)
#scatter(L_2500s, alpha_oxs, s=10.0*angles, markers=markers)
#scatter(L_2500s[0], alpha_oxs[0], edgecolors=colors[0], facecolors="None",marker="s", s=400, label="H13", lw=2)
#scatter(L_2500s[1], alpha_oxs[1], edgecolors=colors[1], facecolors="None",marker="s",  s=400, label="This work", lw=2)


logl2500_line = np.arange(27.5,33.5,0.5)
alpha_ox_line = -0.137 * logl2500_line + 2.638
plot(10.0**logl2500_line, alpha_ox_line, linestyle="--", c="k")
semilogx()
ylim(-3.1,-1.0)
xlim(10.0**27.5,10.0**32.5)
#xlim(10.0**23,10.0**28.4)

ylabel(r"$\alpha_{{\rm ox}}$", fontsize=20)

#clf()
subplot(212)

scatter(10.0**l2500, 10.0**lx, c="k", s=40, label="Steffen+ 2006, COMBI-7")
scatter(10.0**dat2[0].astype(np.float), 10.0**dat2[1].astype(np.float), edgecolors="k", s=40, facecolors="None", label="Steffen+ 2006, BQS")
scatter(10.0**bal_l2500, 10.0**bal_l2, marker="^", edgecolors="#44AA99", facecolors="None", s=150, lw=2, label="Saez+ 2012, BALQSO")
...
for jj in range(2):
	for i in range(len(angles[jj])):
		...
		text(L_2500s[jj][i]-(L_2500s[jj][i]*0.13), Lxs[jj][i]-(Lxs[jj][i]*0.15), angles[jj][i], color=colors[jj],fontsize=16, bbox=bbox_props[jj])
		#else:
...
plot(10.0**logl2500_line, 10.0**loglx_line, linestyle="--", c="k")
...
scatter([0,0],[0,0], edgecolors=colors[0], facecolors="None",marker="s", s=100, label="H13", lw=2)
scatter([0,0],[0,0], edgecolors=colors[1], facecolors="None",marker="s",  s=100, label="This work", lw=2)

loglog()
ylim(10.0**22,10.0**28.4)
xlim(10.0**27.5,10.0**32.5)


xlabel("$L_{2500}$", fontsize=20)
ylabel("$L_{2kev}$", fontsize=20)

subplots_adjust(hspace=0)

legend(loc=2, scatterpoints = 1)

savefig("alpha_ox_%s.png" % label, bbox="tight")

#savefig("../../alpha_ox_both.png", bbox="tight")
